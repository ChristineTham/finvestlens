...
| Navigating **to the Dashboard** blocks the main thread | FR-DOC-01 | Measured 15 Jul 2026 (debug): clearing a find that returns to the Dashboard pegs one core at **100% for ~7s**, and the window shows the stale previous view for the whole of it — the click looks ignored, which is exactly how it was misread as a dead button. The same Clear returning to a *register* costs a 35% blip under 1s. So the cost is the Dashboard rebuild (net-worth history over 46,553 transactions), not Find, Clear, or the search machinery; any path to the Dashboard should pay it. Release should be ~4× better (~1.75s) but that is still a visible stall on a click. Fix is the `DocumentLoader` treatment: derive the dashboard off the main actor, or cache it behind `derivedRevision` like `priceRows`. | P7 |
| The **main-actor tail of an open** is most of the open | FR-DOC-01 | Found by building the progress bar, 15 Jul 2026. The bar meters `store.read()`, which measures **5.6s** (debug) — but a whole open is **~23s of 100% CPU** on this book. The other ~17s is everything after the read: `reloadKvpCollections` + `refreshAll` (account tree, balances, search) and then SwiftUI rendering the dashboard. All of it is on the main actor, so it cannot report progress on the thread it would report on, and the window cannot repaint: the bar necessarily sits at 100% under "Preparing your accounts…" for the whole of it. The read was the part already fixed (`DocumentLoader`, §12.6); the tail is the part left, and it is the bigger half. Same fix as the Dashboard row above — this is that row's cost, paid on every open rather than every Clear. | P7 |
