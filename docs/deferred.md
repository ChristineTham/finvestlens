...
| ~~Register ops absent~~ | **Mostly done (16 Jul 2026):** Cut/Copy/Paste (⇧⌘X/C/V — shifted, since ⌘C belongs to whatever text has focus), Go to Date (⌘G), Schedule-from-transaction, and the Auto-Split Ledger style. Still absent: **Blank-transaction in-register entry** (the register is a read-only `Table`; entry is sheet-only) and **Edit Exchange Rate on an existing txn** (`EditableSplit.quantity` — the field that carries an FX leg's foreign amount — is carried through but has no field of its own). | mostly / P8 |
